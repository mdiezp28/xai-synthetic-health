...
def _build_feature_groups(output_info_list):
    """
    Build slice indices for grouping expanded features back to original columns.
    
    For example, a categorical column with 5 categories becomes 5 one-hot features.
    This function creates slices to group them back: [slice(0,5), slice(5,8), ...]
    """
    groups = []
    start_idx = 0
    ...
    for col in output_info_list:
        ...
        groups.append(slice(start_idx, start_idx + span))
        start_idx += span
    return groups
...
class Generator(Module):
    """
    Generator with an optional SHAP-based focus signal.

    Args:
        embedding_dim (int): Size of the random noise input.
        generator_dim (tuple): Hidden layer sizes (Residual blocks).
        data_dim (int): Output dimension (transformed data).
        focus_dim (int): Size of the focus signal appended to the input.
            Set to 0 to disable (default behaviour, fully backward-compatible).
    """

    def __init__(self, embedding_dim, generator_dim, data_dim, focus_dim: int = 0):
        super(Generator, self).__init__()
        self.focus_dim = focus_dim
        dim = embedding_dim + focus_dim          # ← focus signal widens the input
        seq = []
...
class DPCGANSynthesizer(BaseSynthesizer):
    ...
    def __init__(self, embedding_dim=128, generator_dim=(256, 256), discriminator_dim=(256, 256),
        ...
        #                 broadcast to [batch, data_dim] before being fed to G.
        # _focus_col_signal : FloatTensor [n_cols] – per-column SHAP importance
        #                     (used for logging / diagnostics).
        self._focus_signal = None          # shape: [data_dim]
        self._focus_col_signal = None      # shape: [n_original_cols]


    # ──────────────────────────────────────────────────────────────────────────
    # Focus-signal helpers
    # ──────────────────────────────────────────────────────────────────────────
    ...
    def _update_focus_signal(self, I_tn: torch.Tensor, threshold: float = -0.01) -> None:
        ...
        # EMA smoothing
        ema_alpha = 0.7
        self._focus_signal = ema_alpha * new_signal + (1 - ema_alpha) * self._focus_signal
    ...
    def fit(self, train_data, discrete_columns=tuple(), epochs=None):
        ...
        prefetcher = None  # Disabled: threading caused contention
        ...
        with open(filename, 'w') as loss_file:
            ...
            for i in epoch_iterator:
                ...
                for id_ in range(steps_per_epoch):
                    ...
                    for n in range(self._discriminator_steps):
                        ...
                        if condvec_pair is None:
                            ...
                            fakez = torch.cat([fakez, c_pair_1], dim=1)  # categories (unique value count) + continuous (1+n_components)
                        ...
                        if i > 0 and i % self.focus_update_interval == 0 and id_ == 0 and n == 0:
                            ...
                            if tn is not None and self.focus_threshold is not None:
                                ...
                                if self._focus_col_signal is not None:
                                    focus_dict = {
                                        column_names[j]: float(self._focus_col_signal[j])
                                        for j in range(min(len(column_names),
                                                          self._focus_col_signal.shape[0]))
                                    }
                                    writer.add_scalars("focus/column_weights", focus_dict, i)
                                    writer.add_scalar(
                                        "focus/n_active_features",
                                        float((self._focus_signal > 0).sum()),
                                        i
                                    )
                    ...
                    # ----- TensorBoard G
                    writer.add_scalar("loss/generator", float(loss_g.detach().cpu()), i)
                ...
                if i == 0:
                    ...
                    print(
                        f"  Data fetching:  {disc_data:6.2f}s ({100 * disc_data / total_time:5.1f}%) - avg {disc_data / len(timing_stats['disc_data_fetch']) * 1000:6.1f}ms")
                    print(
                        f"  Forward pass:   {disc_fwd:6.2f}s ({100 * disc_fwd / total_time:5.1f}%) - avg {disc_fwd / len(timing_stats['disc_forward']) * 1000:6.1f}ms")
                    print(
                        f"  Backward pass:  {disc_bwd:6.2f}s ({100 * disc_bwd / total_time:5.1f}%) - avg {disc_bwd / len(timing_stats['disc_backward']) * 1000:6.1f}ms")
                    print(f"\nGENERATOR TRAINING:")
                    print(
                        f"  Data fetching:  {gen_data:6.2f}s ({100 * gen_data / total_time:5.1f}%) - avg {gen_data / len(timing_stats['gen_data_fetch']) * 1000:6.1f}ms")
                    print(
                        f"  Forward pass:   {gen_fwd:6.2f}s ({100 * gen_fwd / total_time:5.1f}%) - avg {gen_fwd / len(timing_stats['gen_forward']) * 1000:6.1f}ms")
                    print(
                        f"  Cond loss calc: {cond_loss:6.2f}s ({100 * cond_loss / total_time:5.1f}%) - avg {cond_loss / len(timing_stats['cond_loss_pair']) * 1000:6.1f}ms")
                    print(
                        f"  Backward pass:  {gen_bwd:6.2f}s ({100 * gen_bwd / total_time:5.1f}%) - avg {gen_bwd / len(timing_stats['gen_backward']) * 1000:6.1f}ms")
                    print(f"\nSUMMARY:")
                    ...
                    print(
                        f"  Data overhead:  {disc_data + gen_data:6.2f}s ({100 * (disc_data + gen_data) / total_time:5.1f}%)")
                    ...
                    if prefetcher and len(timing_stats['queue_size']) > 0:
                        ...
                        print(
                            f"  Queue size: min={min(queue_sizes)}, avg={np.mean(queue_sizes):.1f}, max={max(queue_sizes)} (out of 8)")
    ...
    def set_device(self, device):
        ...
        if self._focus_signal is not None:
            self._focus_signal = self._focus_signal.to(self._device)
